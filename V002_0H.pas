//Metodo_OV_002_0  WIN - S/ BT e TT ticks para WINFUT

input
	fatorBE(1.75);   		//Fator para determinar se é uma BE
	horaSaida(1000);	//Horário de saída de todas as operações
	qnt(2); 			//Quantidade inicial de contratos/lotes
	ticksLucro(30); 	//Ticks para pegar lucros parcial 1
	ticksLucro2(45); 	//Ticks para pegar lucros parcial 2
	ticksStop(1);		//Ticks para stop além da abertura da BE
	BEmin(35);			//Tamanho mínimo BE
	BEmax(80);			//Tamanho máximo BE
	maxStop(80);		//Ticks para limite máximo da ordem de stop

	fatorBT(2);         //Fator para determinar a proporção do corpo/pavio

	BImax(17);           //Ticks para determinar tamanho máximo da barra ignorada
...
begin
	...
	if Time < horaSaida then
		begin
			if (barraBEbear or clearBarBear or bear180 or GBI) then	//BEAR
				begin
					Paintbar(clFuchsia);
					if abaixo200 and (not HasPosition) then //BULL
						begin
			...
			if (barraBEbull or clearBarBull or bull180 or RBI) then
				begin
					Paintbar(clGreen);
